...
function updateEnv(variable, value) {
  ...
  fs.readFile(envFilePath, "utf8", (err, data) => {
    if (err) {
      console.log("Cannot read .en filev:", err);
      return;
    }
...
function isExpiredToken(token) {
  ...
  try {
    ...
    if (parsedPayload.exp) {
      ...
      this.log(colors.cyan(`Has the token expired? ${isExpired ? "Yes, you need to replace the token" : "Not yet..go full throttle"}`));
      return isExpired;
    } else {
      this.log(colors.yellow(`Perpetual Token Unreadable Expiry Time`));
      return false;
    }
  } catch (error) {
